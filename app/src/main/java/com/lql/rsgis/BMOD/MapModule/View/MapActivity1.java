package com.lql.rsgis.BMOD.MapModule.View;
...
import com.lql.rsgis.BMOD.SystemModule.AboutActivity;
import com.lql.rsgis.BMOD.SystemModule.LockviewActivity;
...
public class MapActivity1 extends BaseActivity {
    ...
    private TextView titleTextView,title;//工程标题
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setText(DirName);//显示工程文件夹名称
...
//        toolbar.setNavigationIcon(null);//设置不显示回退按钮
        getLayoutInflater().inflate(R.layout.activity_toobar_view, toolbar);
        title = (TextView) toolbar.findViewById(R.id.activity_baseview_toobar_view_txtTitle);
        //判断是否为平板设备
        if (context!=null){
            boolean ispad = SysUtils.isPad(context);
            if (ispad){
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        MenuItem reItem= menu.add(Menu.NONE, Menu.FIRST + 1, 0, "显示待办");
...
//                MainActivity.RefreshTask refreshTask =new MapActivity1().RefreshTask(context);
//                refreshTask.execute();
                break;
